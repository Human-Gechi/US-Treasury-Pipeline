...
BASE_API_URL = (
    "https://us-treasury-pipeline.onrender.com"  # Deployed url on render
)
...
def fetch_security_types():
    payload = request_json(
        f"{BASE_API_URL}/records/types"
    )  # Call the API endpoint
    if not payload:
        return []
    return (
        payload.get("Security_type_desc", [])
...
def card_display():
    ...
    for t in types:  # For each security type
        recs = (
            fetch_records_for_type(t) or []
        )  # Fetch records for that security type
        counts[t] = (
    ...
    for _ in range(rows):
        ...
        for col in cols:
            if idx >= len(
                types
            ):  # Break if number of types > types of security types
                break  # Break
            t = types[idx]  # Get the security type at the current index
            with col:
                st.metric(
                    label=t, value=counts.get(t, 0)
                )  # Display the security type and its count in a metric card
...
def line_graph_filtered():
    ...
    with col3:
        ...
        for security_type in selected_types:  # For each security type selected
            records = (
                fetch_records_for_type(security_type) or []
            )  # Fetch the record
            for rec in records:
        ...
        if (
            ...
            or "avg_interest_rate_amt" not in df.columns
        ):
            st.dataframe(df)
            return
    ...
    if st.session_state.get("line_chart_df") is not None:
        st.subheader(
            "% Securities Average Interest Rate Over Time"  # Subheader for line graph
        )
        st.line_chart(
